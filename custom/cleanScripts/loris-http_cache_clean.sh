...
#
LOG="/var/log/loris/http_cache_clean.log"

# Check that the cache directories...
IMG_CACHE_ROOT_DIR="/usr/local/share/images/"
IMG_CACHE_DP_DIR="/var/cache/loris/"
...
while [ $usage -gt $REDUCE_TO ] && [ $max_age -ge -1 ]; do
        ...
        for f in $(find $IMG_CACHE_ROOT_DIR -name loris_cache.* -type f -amin +$max_age); do
                rm $f
                let delete_total+=1
        done

        # files. loop (instead of -delete) so that we can keep count
        for f in $(find $IMG_CACHE_DP_DIR -type f -amin +$max_age); do
                rm $f
                let delete_total+=1
        done
